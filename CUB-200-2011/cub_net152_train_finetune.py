...
import data.cub_data as data
import model.cub_resnet152 as MSHQP

os.environ["CUDA_VISIBLE_DEVICES"] = "2"

trainset = data.MyDataset('data/cub_trainlist_shuffle.txt', transform=transforms.Compose([
...
testset = data.MyDataset('data/cub_testlist.txt', transform=transforms.Compose([
                            transforms.Resize((600, 600), Image.BILINEAR),
...
pred_dict = torch.load('firststep_cub_net152.pth')
...
optimizer = optim.SGD([{'params': model.features.parameters(), 'lr': 1e-2 * lr},
                       {'params': model.sample_512.parameters(), 'lr': 1e-2 * lr},
                       {'params': model.sample_1024.parameters(), 'lr': 1e-2 * lr},
                       {'params': model.sample_2048.parameters(), 'lr': 1e-2 * lr},
                       {'params': model.feature_select_weight, 'lr': 1e-4 * lr},
                       {'params': model.bn.parameters(), 'lr': 1e-2 * lr},
                       {'params': model.fc_concat.parameters(), 'lr': 1e-2 * lr},
...
torch.save(model.state_dict(), 'cub_result_net152.pth')
