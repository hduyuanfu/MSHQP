...
import data.car_data as data
import model.car_resnet152 as MSHQP
from utils.utils import progress_bar
os.environ["CUDA_VISIBLE_DEVICES"] = "0"
trainset = data.MyDataset('data/car_trainlist_shuffle.txt', train=True, transform=transforms.Compose([
...
trainloader = torch.utils.data.DataLoader(trainset, batch_size=6, shuffle=True, num_workers=6)
testset = data.MyDataset('data/car_testlist.txt', train=False, transform=transforms.Compose([
                            transforms.Resize((500, 500), Image.BILINEAR),
...
pred_dict = torch.load('firststep_car_net152.pth')
...
lr = 1.0
model.is_init = True
optimizer = optim.SGD([{'params': model.features.parameters(), 'lr': 1e-2 * lr},
                       {'params': model.sample_512.parameters(), 'lr': 1e-1 * lr},
                       {'params': model.sample_1024.parameters(), 'lr': 1e-1 * lr},
                       {'params': model.sample_2048.parameters(), 'lr': 1e-1 * lr},
                       {'params': model.feature_select_weight, 'lr': 1e-4 * lr},
                       {'params': model.bn.parameters(), 'lr': 1e-1 * lr},
                       {'params': model.fc_concat.parameters(), 'lr': 1e-1 * lr},
                       {'params': model.fc_sum.parameters(), 'lr': 1e-3 * lr},
], lr=0.1, momentum=0.9, weight_decay=1e-5)
...
torch.save(model.state_dict(), 'car_result_net152.pth')
